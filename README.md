...
| nvim (hendrikmi) | hendrikmi 심링크 (default) | `init.lua`/`debug.lua` 로컬 수정 + [`overrides/nvim/after/ftplugin`](overrides/nvim/after/ftplugin) — wezterm preview off, dap-python Python 경로 명시, bufferline 활성화, markdown/text wrap. [`overrides/nvim/lua/plugins/lsp.lua`](overrides/nvim/lua/plugins/lsp.lua) — pylsp 제거 → basedpyright (`.venv` 자동 감지) + ruff hover off. [`overrides/nvim/lua/plugins/debug.lua`](overrides/nvim/lua/plugins/debug.lua) — DAP 디버그 키맵 `<space>5~9` (continue/into/over/out/breakpoint) |
